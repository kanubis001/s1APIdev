...
class mainReporting:
    def __init__(self, url, gid, token, st, en, stype, names, path):
        self.s1Site = url
        # self.apitoken = apitoken
        self.groupIds = gid
        self.token = token
        self.startD = str(st)+"T00:00:00.000000Z"
        self.endD = str(en)+"T23:59:59.999999Z"
        self.stype = stype
        self.rootpath=path
        if stype == "site":
            self.makeReport = mkReport.mkReport(st, en, [names[0], names[1]],self.rootpath)
        else:
            self.makeReport = mkReport.mkReport(st, en, names,self.rootpath)
            ...
            if type == 'all':
                ...
                for data in datas:
                    if type in data:
                        ...
                        #       "/", data[16], "/", data[14])
                        cont=f"{data[15]}/{data[10]}/{data[16]}/{data[14]}"
                        # cont=str(data[15]+"/"+data[10]+"/"+data[16]+"/"+data[14])
                        log_write2(sys, f"type not all : {cont}")
            # bar.finish()
            return 1
    ...
    def gatherByEngine(self, datas):
        try:
            ...
            # print("Value err!")
            log_write2(sys,"6. 엔진별 탐지 Value err!")

        except TypeError:
            log_write2(sys,"6. 엔진별 탐지 Type err!")
