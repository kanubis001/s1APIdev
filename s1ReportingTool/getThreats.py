...
class getThreats:
    ...
    def getfromDate(self, startD, endD, searchType,id):
        self.ids=id
        getUrl = self.uri+"updatedAt__gt="+startD+"&updatedAt__lt="+endD
        if searchType == "group":
    ...
    def saveDataArr(self, contents):
        ...
        for i in range(init, end):
            try:
                ...
                # print(self.totcnt)
                # print(dataFcont.get("agentRealtimeInfo"))
                ...
                # self.makeReport.writeDownTable(self.dataArr[self.totcnt],self.totcnt)
                self.totcnt += 1
